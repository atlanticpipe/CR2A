document.addEventListener("DOMContentLoaded", () => {
  ...
  const detectApiBase = () => {
    // Resolve API base from explicit override, meta tag, or same-origin host; always return a non-empty string.
    ...
    const overrides = [window.CR2A_API_BASE, meta, htmlData].filter((v) => !!v && v !== "undefined" && v !== "null");
    if (overrides.length) return String(overrides[0]).replace(/\/$/, "");
    const origin = window.location.origin || "";
    const base = origin.startsWith("http") ? origin.replace(/\/$/, "") : "/api";
    return base || "/api";
  };
  ...
  const probeBackend = async () => {
    // Probe /health to find a reachable API; fall back to the first usable candidate even if /health fails.
    const origin = window.location.origin || "";
    const candidates = new Set();
    if (API_BASE_URL) candidates.add(API_BASE_URL);
    if (origin.startsWith("http")) {
      const cleanOrigin = origin.replace(/\/$/, "");
      candidates.add(cleanOrigin);
      candidates.add(`${cleanOrigin}/api`);
    }
    candidates.add("/api");

    const ordered = Array.from(candidates).filter(Boolean);
    const fallback = (ordered[0] || "/api").replace(/\/$/, "");

    updateBackendHelper(fallback, "probing");
    ...
    updateBackendHelper(API_BASE_URL, "health check failed; using fallback");
  };

  probeBackend().catch(() => updateBackendHelper(API_BASE_URL || "/api", "probe failed"));
