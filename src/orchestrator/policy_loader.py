# -*- coding: utf-8 -*-
# Policy bundle loader: reads version locks, schemas, and rulebook with integrity checks.
from __future__ import annotations          # future annotations for forward refs (py3.10+)
import json                                 # parse JSON policy files
import hashlib                              # compute/verify SHA-256 checksums
from pathlib import Path                    # OS-safe path handling
from typing import Dict, Any, Union, Optional  # precise typing used throughout
import asyncio                              # stdlib event loop (used by callers elsewhere)
# Import the project’s error handler used for consistent exception reporting
from utils.error_handler import handle_exception  # central error handler
# YAML is optional but needed for the human-readable rulebook; load defensively
try:
    import yaml                             # parse YAML rulebook (requires PyYAML)
except ModuleNotFoundError:                 # fail gracefully if PyYAML isn't installed
    yaml = None                             # callers should check and raise a clear error when needed

# repo-relative locations (fixed typos: 'contract-analysis-policy-bundle')
BUNDLE_FILES: Dict[str, str] = {
    # version lock JSON used to pin all bundle components
    "version_lock": "contract-analysis-policy-bundle/policy/version_lock.schemas_v1.json",
    # human-readable YAML rulebook defining high-level logic
    "rulebook": "contract-analysis-policy-bundle/policy/rulebook_v1.yaml",
    # validation rules enforcing schema and policy consistency
    ...
    # run manifest schema used to structure run metadata
    "run_manifest_schema": "contract-analysis-policy-bundle/schemas/run_manifest_schema_v1.json",
}

class PolicyBundle:
    """Small helper around the policy bundle folder with memoized reads and integrity checks."""
    def __init__(self, root: Union[str, Path]) -> None:
        self.root: Path = Path(root)  # repo root path (ensures uniform Path object)
        self.paths: Dict[str, Path] = {
            k: self.root / v          # map logical key to its absolute path
            for k, v in BUNDLE_FILES.items()
        }
        self._cache: Dict[str, Any] = {}  # memoized file contents for faster reuse
