...
# Safely attempt to import python-magic
_magic_spec = importlib.util.find_spec("magic")
magic = importlib.import_module("magic") if _magic_spec is not None else None
...
def infer_mime_type(file_path: str | Path) -> str:
    ...
    # Use python-magic if available for reliable detection
    if magic is not None:  # pragma: no cover - depends on native libmagic
        try:
            ...
            # Fall through to extension-based guess if magic fails
            pass
    ...
    raise ValueError(f"Unable to determine MIME type from content or extension: {path}")
