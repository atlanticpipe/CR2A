...
DEFAULT_SCHEMA = DEFAULT_ROOT / "schemas" / "output_schemas.json"
DEFAULT_RULES = DEFAULT_ROOT / "schemas" / "validation_rules.json"
...
def cmd_validate(args: argparse.Namespace) -> int:
    """Validate a filled template JSON against schema and rules."""
    ...
    if not output_path.exists():
        logging.error("Output JSON not found: %s", output_path)
        return 2
    if not schema_path.exists():
        logging.error("Schema file not found: %s", schema_path)
        return 2
    if not rules_path.exists():
        logging.error("Validation rules not found: %s", rules_path)
        return 2
...
def cmd_analyze(args: argparse.Namespace) -> int:
    """Analyze a contract document into filled CR2A JSON."""
    try:
    ...
    out_path = (
        ...
        if args.output_json
        else Path("filled_template.from_input.json")
    )
...
def cmd_export_pdf(args: argparse.Namespace) -> int:
    """Export a filled template JSON to PDF report."""
    try:
    ...
    output_pdf = (
        ...
        if args.output_pdf
        else Path("cr2a_export.pdf")
    )
    template_docx = (
        Path(args.template_docx).expanduser().resolve()
        if args.template_docx
        else DEFAULT_TEMPLATE
    )

    if not input_json.exists():
        logging.error("Input JSON not found: %s", input_json)
        return 2
    if args.backend == "docx" and not template_docx.exists():
        logging.error("Template DOCX not found: %s", template_docx)
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap = argparse.ArgumentParser(
        description="CR2A CLI - Contract Risk & Compliance Analysis"
    )
    ...
    a.add_argument(
        "--ocr",
        default=os.getenv("OCR_MODE", "auto"),
        choices=["auto", "textract", "tesseract", "none"],
    ...
    a.add_argument(
        ...
        default="off",
        choices=["off", "on"],
        help="Enable OpenAI-assisted refinement (requires OPENAI_API_KEY or OPENAI_SECRET_ARN)"
    )
    a.add_argument(
        "--max-file-mb",
        type=float,
        default=MAX_FILE_MB_DEFAULT,
    ...
    e.add_argument(
        "--backend",
        default="docx",
        choices=["reportlab", "docx"],
        help="Export backend"
    )
    e.add_argument(
        "--template-docx",
        default=str(DEFAULT_TEMPLATE),
        help="Optional DOCX template path for backend=docx"
    )
