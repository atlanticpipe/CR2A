...
GUI_CONFIG = BuildConfig(
    ...
    hidden_imports=[
        ...
        "src.analysis_engine",
        "src.analysis_models",
        "src.backend_registry",
        "src.bid_review_engine",
        "src.bid_review_models",
        "src.bid_review_tab",
    ...
    collect_packages=["src", "PyQt5", "tokenizers", "cryptography", "llama_cpp", "rapidfuzz", "PIL", "pdf2image", "pytesseract", "anthropic"],
    excludes=[
...
class SpecGenerator:
    ...
    def generate(self, config: BuildConfig) -> str:
        ...
        # Ensure project root is on sys.path so local packages (src, analyzer)
        # are importable by collect_submodules/collect_data_files
        lines.append(f"import sys")
        lines.append(f"sys.path.insert(0, r'{str(self.project_root).replace(chr(92), '/')}')")
        lines.append("")
